    ...
    Protocol = module.exports = function(client, con) {
        ...
        this.authenticate = function(model, options, next) {
            ...
            Store.has(lookup, function(err, exists) {
                if (!exists) {
                    options.error && options.error('User not found.', model, options);
                    return;
                } 
                else {
                    // Check password
                    Store.get(lookup, function(err, val) {
                        ...
                        if (matched !== val) {
                            options.error && options.error('Invalid password.', model, options);
                            return;
                        } 
                        else {
        ...
        this.register = function(model, options, next) {
            ...
            Store.has(lookup, function(err, exists) {
                if (exists) {
                    options.error && options.error('Username taken.', model, options);
                    return;
                }
        ...
        this.restricted = function(model, options, next) {
            ...
            options.error && options.error('Authentication required.', model, options);
            return;
        };
    };
})()