(function() {
    ...
    Protocol = module.exports = function(client, con) {
        ...
        con.on('ready', function() {
            console.log('Ready: ', con.id);
            clients[con.id] = client;
        });
        
        // Client disconnected
        con.on('end', function() {
            console.log('Ending connection: ', con.id);
            delete clients[con.id];
        });
        
        // Channel subscription
        this.subscribe = function(model, options, cb) {
            console.log('Subscribing: ', JSON.stringify(model));
        ...
        this.publish = function(model, options, cb) {
            console.log('Publishing: ', JSON.stringify(model));
        ...
        this.create = function(model, options, cb) {
            console.log('Creating: ', JSON.stringify(model));
        ...
        this.update = function(model, options, cb) {
            console.log('Updating: ', JSON.stringify(model));
        ...
        this.destroy = function(model, options, cb) {
            console.log('Destroying: ', JSON.stringify(model));
